...
- On 2026-08-12 the authorized cutover fast-forwarded local `main` to
  `b7bebe5b`, backed up and hardened the installed LaunchAgent with
  `--no-proxy-headers`, rebuilt Next 16.3, and restarted only the existing
  dashboard service while leaving Cloudflare running. The release passed live
  EN/FR, auth, cap, cache, throttle, responsive, and visual checks at the
  recovered public tunnel. The rollback ref is
  `backup/main-pre-dashboard-redesign-20260812-463d919a`.
- `origin/main` was deliberately not pushed: Render authority remains expired
  and its reachable hostname appears to serve an older Streamlit build, so a
  push could create an uncoordinated second production. The missing Keychain
  entry also remains a credential-ownership hardening follow-up.
