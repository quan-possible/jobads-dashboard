...
def _pages() -> list[Section]:
    return [
        Section("1", "Pulse — the market's vital signs", "pulse",
            ...
            Item(pulse.kpi_row, "Four headline numbers with 24-month sparklines anchor the page.", "Core"),
            ...
            Item(pulse.stl_panel, "Trend, season, and shock pulled apart — COVID lands in the remainder.", "Deep"),
            Item(pulse.anomaly_flags, "A handful of months deviate sharply from the seasonal expectation.", "Deep"),
            Item(pulse.sa_vs_nsa, "Seasonally-adjusted view removes the calendar rhythm.", "Deep"),
            Item(pulse.diffusion_index, "Breadth: how many groups grow at once, not just the total.", "Deep"),
            Item(pulse.cycle_plot, "Each month's own trajectory across the years.", "Deep"),
        ]),
        Section("2", "Geography — where, and what each place is known for", "geography",
            ...
            Item(geography.bubble_map, "Ontario, Quebec, BC and Alberta dominate raw volume.", "Core"),
            Item(geography.ranked_provinces, "The list carries the exact ranking the map cannot.", "Core"),
            Item(geography.share_choropleth, "Share of national demand, normalised — never raw counts on a map.", "Core"),
            Item(geography.lq_choropleth, "Location quotient shows relative specialisation, not size.", "Deep"),
            Item(geography.lq_heatmap, "A dense province × occupation specialisation wall.", "Deep"),
            Item(geography.shift_share_bars, "Is a province's change structural or competitive? (identity, not cause).", "Deep"),
            Item(geography.yoy_choropleth, "Where momentum is positive vs cooling.", "Deep"),
            Item(geography.province_tile_grid, "Equal-area cells keep the North as legible as Ontario.", "Deep"),
        ]),
        Section("3", "Occupations — what work, and how the mix is shifting", "occupations",
            ...
            Item(occupations.treemap, "Sales & service and trades carry the largest volume.", "Core"),
            ...
            Item(occupations.bump_chart, "Rank journeys — who overtook whom since 2017.", "Deep"),
            Item(occupations.noc_naics_heatmap, "Which sectors demand which occupations.", "Deep"),
            Item(occupations.concentration_trio, "Three lenses on whether demand is concentrating.", "Deep"),
            Item(occupations.horizon_wall, "Every group's trajectory folded into one screen.", "Deep"),
        ]),
        Section("4", "Industries (NAICS) — coverage-gated", "industries",
            ...
            Item(industries.treemap, "Sector volumes among coded postings.", "Core"),
