...
- The date-range control keeps the slider, but now wraps it with a custom header containing start/end month pills; Streamlit's built-in slider label, thumb values, and tick labels are hidden to avoid cramped duplicate text.
...
- On 2026-06-03, the Mac Mini public website was repaired for normal Explore usage and posting-level snippet lookup. The public app on port `8522` includes `posting_lookup.parquet`, the Explore posting lookup searches through the full selected end month, and the UI has an explicit `Search postings` button so users do not have to rely on Streamlit's Enter-to-apply behavior.
- Verification for the 2026-06-03 recovery: local focused tests passed with `14 passed`, remote focused tests on `bruces-mac-mini` passed with `14 passed`, `http://127.0.0.1:8522/_stcore/health` returned `ok`, and the public URL `https://047e-2001-56a-f068-c900-dc77-45fd-2bfb-31da.ngrok-free.app` returned posting `65563604` with a `Server` row and bounded description excerpt through the browser.
- A follow-up 2026-06-03 change expanded the posting lookup bundle with `description_full` and updated Explore so the selected posting details show `Full description` when available. The results table and row limits remain bounded; full text is shown only in the selected detail panel.
- Verification for the full-description follow-up: local focused tests passed with `22 passed`, remote focused tests on `bruces-mac-mini` passed with `15 passed`, the enriched public lookup file is 105,604,302 bytes, posting `65563604` has a 2,247-character `description_full`, Mini health returned `ok`, and the public browser showed `Full description` with responsibilities/qualifications text for posting `65563604`.
- The Mac Mini public website now requires a password before dashboard data loads. The public LaunchAgent has `JOBADS_DASHBOARD_AUTH_REQUIRED=true` and stores only a salted PBKDF2 password hash in its environment; do not commit the password or hash into the repo.
- Verification for the password gate: local focused app/posting tests passed with `16 passed`, remote focused tests on `bruces-mac-mini` passed with `16 passed`, Mini health returned `ok`, and the public Browser confirmed the live URL shows a restricted password screen, rejects a wrong password, unlocks with the correct password, and still runs Explore -> Specific postings for posting `65563604`.
- A 2026-06-03 sidebar toggle follow-up replaced the visible `Open filters` / `Close filters` labels with a centered icon-only chevron, hid Streamlit's native Material SVG inside the control, and made the Streamlit header transparent to pointer events so it no longer intercepts the filter button.
- Verification for the sidebar toggle follow-up: `PYTHONPATH=src pytest -q tests/test_app_surface.py` passed with `11 passed`, `python -m py_compile src/jobads_dashboard/dashboard/app.py` passed, local `127.0.0.1:8520` and Mini tunnel `127.0.0.1:8521` both toggled from the marked button coordinate, and the Mac Mini public/private health endpoints returned `ok` after redeploy/restart.
- The public dashboard password is saved in macOS Keychain under service `jobads-dashboard-public-password` and account `jobads-dashboard-public`; `AGENTS.md` now includes the retrieval command so future agents can find it without storing the plaintext secret in repo files.
- On 2026-06-11, the public LaunchAgent was repointed to serve the dashboard from `/Volumes/ACLMR/jobads-dashboard` with data root `/Volumes/ACLMR/jobads-dashboard/data/derived/labor_market_dashboard_v1`; it still uses the working local Python wrapper at `/Users/brucenguyen/jobads-dashboard/.venv/bin/python` because launchd cannot read the external-drive venv's `pyvenv.cfg`.
...
- The `Explore` tab remains a bounded query surface. It can show full indexed text for one selected posting, but arbitrary SQL, bulk raw-text export, and `ai-labor` research-output ingest still require separate governance and derived-table design before exposure.
