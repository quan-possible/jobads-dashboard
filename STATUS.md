...
- The active checkout is `feat/login-uncapped`, tracking `origin/feat/login-uncapped`. It has diverged from `origin/main`, which has 10 commits absent from the feature branch. The published branch contains the authenticated uncapped-chart work, public Explore-nav restriction, Claude Design sync inputs, and current harness migration.
...
1. Decide how to integrate the 10 `origin/main`-only commits, then whether and when to merge `feat/login-uncapped`; the feature work is published but remains outside `main`.
...
- The feature branch is published but remains 10 commits behind and unmerged into `origin/main`, so canonical promotion requires deliberate integration.
