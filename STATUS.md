...
- The redesign is live through the existing Mac public stack: FastAPI on `127.0.0.1:8530`, Next 16.3.0 build `Yr_bsL_RHqDGbg96bezak`, and the unchanged Cloudflare quick tunnel at `https://topics-marion-although-restore.trycloudflare.com`. The tunnel hostname is ephemeral and must still be recovered from its log after any future tunnel restart.
...
1. Resolve the Render service ownership/auto-deploy boundary before pushing the local `main` commits to `origin/main`; the local Cloudflare-backed production release is already complete.
