...
import { fetchPostings } from "@/lib/explore";
...
function rowWage(r: PostingRow): string {
  if (r.wage_hourly != null) return `${fmtWage(r.wage_hourly)}`;
  if (r.wage_min != null && r.wage_max != null) return `${fmtWage(r.wage_min)}–${fmtWage(r.wage_max)}`;
  if (r.wage_min != null) return `${fmtWage(r.wage_min)}+`;
  return "—";
}
...
export function ExploreView() {
  ...
  // Scope comes from the shared FilterSpine (URL). Reset paging when it changes.
  ...
  useEffect(() => {
    ...
    fetchPostings({
      ...
      .catch((e) => !cancelled && setError(e?.message ?? "Could not load postings."))
      .finally(() => !cancelled && setLoading(false));
    return () => {
      cancelled = true;
    };
  }, [filters.geo, filters.occ, filters.ind, debouncedQ, offset]);
            ...
            <tbody>
              ...
              {rows.map((r) => (
                <tr
                  ...
                  className="cursor-pointer border-b border-hairline transition-colors last:border-0 hover:bg-orange/[0.04] focus:bg-orange/[0.06] focus:outline-none"
                >
                  <td className="num whitespace-nowrap px-4 py-3 text-ink-soft">{r.date_found ? fmtMonth(r.date_found) : fmtMonth(r.month)}</td>
                  ...
                  <td className="num whitespace-nowrap px-4 py-3 text-right text-navy">{rowWage(r)}</td>
