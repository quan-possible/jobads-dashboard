...
5. Cap every chart/visual at **10 distinct categories/items** (bars, treemap tiles, pie slices, heatmap rows or columns, legend line series, labelled scatter points, choropleth fill values). When a dimension has more, **group the surplus to preserve totals** — fold the long tail into a residual "Other" bucket, lower the top-N for inherently-ranked charts, or use a meaningful regional grouping (e.g. the four Atlantic provinces → "Atlantic Canada"). Never silently drop categories. **Time axes are exempt** (e.g. the 12 months of the seasonality heatmap). Shared helpers already exist: `cap_other`/`cap_columns`/`MAX_CATEGORIES` in `viz/figures/_common.py`, the province region map in `viz/datasource.py`, and `_cap_bar` in `api/explore.py`.
