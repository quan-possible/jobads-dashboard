...
_FR_CHROME: dict[str, str] = {
    # axis titles
    "occupation groups with annual growth (%)": "groupes professionnels en hausse sur un an (%)",
    "postings with an industry code (%)": "offres avec un code sectoriel (%)",
    ...
    "HHI": "IHH",
    "year-over-year posting change": "variation annuelle des offres",
    "advertised hourly wage": "salaire horaire affiché",
    "advertised median wage": "salaire médian affiché",
    "components of posting change": "composantes de la variation des offres",
    "contribution to posting change (percentage points)": "contribution à la variation des offres (points de pourcentage)",
    "cumulative %": "% cumulé",
    "industry": "industrie",
    "occupation share ÷ national share": "part de la profession ÷ part nationale",
    "Occupation": "Profession",
    "Industry": "Industrie",
    "Hourly wage": "Salaire horaire",
    "Remote work": "Télétravail",
    "Posting language": "Langue de l’offre",
    "English requirement": "Exigence en anglais",
    "French requirement": "Exigence en français",
    "Experience detail": "Expérience détaillée",
    "Education": "Scolarité",
    "Skills": "Compétences",
    "Employment type": "Type d’emploi",
    "Duration": "Durée",
    "Advertised by": "Publié par",
    ...
    "change from a year earlier (%)": "variation par rapport à un an plus tôt (%)",
    "index (base year = 100)": "indice (année de base = 100)",
    "postings: 3-month average − 12-month average": "offres : moyenne sur 3 mois − moyenne sur 12 mois",
    "change in postings": "variation des offres",
    "change in skill-mention share (percentage points)": "variation de la part des mentions de compétences (points de pourcentage)",
    ...
    "task-exposure score": "indice d’exposition des tâches",
    ...
    "per 10,000 workers": "pour 10 000 actifs",
    "postings": "offres",
    ...
    "half of groups": "moitié des groupes",
    ...
    "Remaining local component": "Composante locale restante",
    "75th percentile": "75e centile",
    "25th–75th percentile": "25e–75e centile",
    # subplot titles
    "Observed": "Observé",
    "Trend": "Tendance",
    "Seasonal": "Saisonnier",
...
def _fr(s: str) -> str:
    ...
    for en, fr in {
        ...
        if s.startswith(en) and s[len(en):].isdigit():
            return fr + s[len(en):]
    total = re.fullmatch(r"total ([+−-]?\d+(?:\.\d+)?) percentage points", s)
    if total:
        value = total.group(1).replace("-", "−").replace(".", ",")
        return f"total {value} points de pourcentage"
...
_FR_HOVER: dict[str, str] = {
    ...
    "of industry postings": "des offres de l’industrie",
    "change from a year earlier": "variation par rapport à un an plus tôt",
    "from a year earlier": "par rapport à un an plus tôt",
    "25th–75th percentile": "25e–75e centile",
    "75th percentile": "75e centile",
    "25th percentile": "25e centile",
    "percentage points": "points de pourcentage",
    "task-exposure score": "indice d’exposition des tâches",
    "national share": "la part nationale",
    ...
    "3-month average": "moyenne sur 3 mois",
    "of annual average": "de la moyenne annuelle",
    "coverage": "couverture",
    "median": "médiane",
    "provisional": "provisoire",
    "postings": "offres",
